...
public:
    virtual ~MoveFactory(){};
    [[nodiscard]] virtual std::shared_ptr<Move<S>> sample(const std::shared_ptr<const S>& s, const Random& random, double stepSize=1.0) const = 0;
    virtual std::vector<std::shared_ptr<Move<S>>> listMoves(const std::shared_ptr<const S>& s, double stepSize) const {
        return {};
    };
};
...
private:
    ...
    virtual void initialize(const std::shared_ptr<S>& initialSolution, double initialScore) = 0;
...
//    [[nodiscard]] virtual std::shared_ptr<Move<S>> sample(std::shared_ptr<const S> s, const Random& random) const = 0;
    std::shared_ptr<Move<S>> sample(const std::shared_ptr<const S>& s, const Random& random, double stepSize) const override {
       int idx;
    ...
    listMoves(const std::shared_ptr<const S>& s, double stepSize) const override {
        std::vector<std::shared_ptr<Move<S>>> moves;
