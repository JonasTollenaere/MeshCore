...
void runRotation(RenderWidget *renderWidget, WorldSpaceMesh& innerMesh, const WorldSpaceMesh& roughMesh){
    ...
    for(int i=0; i<500; i++){
        ...
        newTransformation = glm::rotate(newTransformation, nextFloat(randomEngine), glm::vec3(nextFloat(randomEngine) - 0.5, nextFloat(randomEngine),nextFloat(randomEngine)));
    ...
    std::cout << stopms - startms << std::endl;
}
...
void test(RenderWidget *renderWidget){
    ...
    const ModelSpaceMesh modelSpaceMesh3 = FileParser::parseFile(R"(C:\Users\tolle\CLionProjects\MeshCore\data\models\banana.stl)");
...
void SandboxTask::run() {
    ...
    const ModelSpaceMesh modelSpaceMesh = FileParser::parseFile(R"(C:\Users\tolle\CLionProjects\MeshCore\data\models\DIAMCADbr1.obj)");
    WorldSpaceMesh worldSpaceMesh = WorldSpaceMesh(modelSpaceMesh);
    renderWidget->addWorldSpaceMesh(worldSpaceMesh, glm::vec4(1,0.0,0.0,1));

    const ModelSpaceMesh modelSpaceMesh5 = FileParser::parseFile(R"(C:\Users\tolle\CLionProjects\MeshCore\data\models\DIAMCADrough.obj)");
    WorldSpaceMesh worldSpaceMesh5 = WorldSpaceMesh(modelSpaceMesh5);
...
//    worldSpaceMesh5.transform(glm::rotate(Transformation(1.0f), 1.57f, glm::vec3(1,0,0)));
    ...
    auto* first = new std::thread(runRotation, renderWidget, worldSpaceMesh, worldSpaceMesh5);
