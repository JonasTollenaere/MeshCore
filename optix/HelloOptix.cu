...
//    2.    Create a pipeline of programs that contains all programs that will be invoked during a ray tracing launch.
        ...
        programGroupDescriptions[0].raygen.entryFunctionName = "__raygen__rg";
        programGroupDescriptions[1].kind = OPTIX_PROGRAM_GROUP_KIND_HITGROUP;
        programGroupDescriptions[1].hitgroup.moduleCH = ptxModule;
        programGroupDescriptions[1].hitgroup.entryFunctionNameCH = "__closesthit__ch";
//        programGroupDescriptions[1].hitgroup.moduleAH = nullptr;
//        programGroupDescriptions[1].hitgroup.entryFunctionNameAH = nullptr;
//      As a special case, the intersection program is not required – and is ignored – for triangle primitives.
//      programGroupDescriptions[1].hitgroup.moduleIS = intersectionModule;
//      programGroupDescriptions[1].hitgroup.entryFunctionNameIS = nullptr;
        programGroupDescriptions[2].kind = OPTIX_PROGRAM_GROUP_KIND_MISS;
        programGroupDescriptions[2].miss.module = ptxModule;
        programGroupDescriptions[2].miss.entryFunctionName = "__miss__ms";
...
//          optixTrace to begin traversal and the execution of the other programs.
        ...
        bool* d_hitPointer = &(reinterpret_cast<HitGroupSbtRecord*>(d_hitGroupRecord))->data.hit;
        ...
        for(int i=0;i<tests;i++){
            CUDA_CHECK(cudaMemcpyAsync(d_hitPointer, &hitGroupRecord.data.hit, sizeof(bool), cudaMemcpyHostToDevice, cuStream));
            OPTIX_CHECK(optixLaunch(pipeline, cuStream, d_optixLaunchParameters, sizeof(OptixLaunchParameters), &sbt, width, 1, 1));
            CUDA_CHECK(cudaMemcpyAsync(&hitGroupRecord.data.hit, d_hitPointer, sizeof(bool), cudaMemcpyDeviceToHost, cuStream));
            CUDA_CHECK(cudaStreamSynchronize(cuStream));
        }
